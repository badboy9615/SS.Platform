...
function initTable(sarcheParam) {
    $('#tt').datagrid({
        url: '/Role/GetAllRoleInfos',//rows=10  page=1
        title: '角色列表',
        width: 690,
        height: 400,
        fitColumns: true,
...
function ShoweditDialog() {
    ...
    ids += rows[0].ID;
    //启动异步请求到后台，查询修改数据
    $.post("/Role/Edit", { ids: ids }, function (data) {
...
function doDelete() {
    ...
    //rows是选中行的数据的json对象的集合
    if (rows.length <= 0) {
        $.messager.alert("错误消息", "删除前请选中要删除的角色？");
        return;
    }

    $.messager.confirm("提示消息", "确认删除选中的角色吗？", function (r) {
        if (r) {
            //把所有行的id拿到
            var ids = "";
            ...
            //启动异步请到后台，批量删除数据
            $.post("/Role/DeleteIds", { ids: ids }, function (data) {
                if (data == "ok") {
                    //清除选中数据。
