...
$(function () {
    ...
    $.getJSON("/Master/GetTreeDepNodes", {}, function (data) {
    ...
    $grid = $("#maingrid").ligerGrid({
        ...
        { display: '用户名', name: 'Name', width: 200 }
        ], width: '100%', pkName: 'ID', pageSize: 20, pageSizeOptions: [5, 10, 15, 20], height: '100%'
        , toolbar: {
            items: [
            { text: '设置负责人', click: setDepartmentUser, icon: 'memeber' },
            { text: '删除负责人', click: DeleteDepartmentUser, img: '../../lib/ligerUI/skins/icons/delete.gif' }
            ]
        }
    });
    //增加用户grid
...
function addSave(item) {
    ...
    //启动异步请求到后台，查询修改数据
    $.post("/Master/AddMaster", { DepId: $selectedDept, UsersId: ids }, function (data) {
        if (data == "ok") {
            //添加成功过了
            $checkedAddUser = [];
            $dlgAdd.hidden();
            $.getJSON("/Master/GetDepartmentMaster", { SDep: $selectedDept }, function (data1) {
                $grid.set({ data: data1 });
            });
        } else {
            //添加失败了
...
function delSave(item) {
    ...
    ids = ids.substr(0, ids.length - 1);
    //启动异步请求到后台，查询修改数据
    $.post("/Master/DelMaster", { DepId: $selectedDept, UsersId: ids }, function (data) {
        if (data == "ok") {
            //添加成功过了
            $checkedDelUser = [];
            $dlgDel.hidden();
...
function treeClick(note) {
    $.getJSON("/Master/GetDepartmentMaster", { SDep: note.data.CategoryId }, function (data) {
        $grid.set({ data: data });
    });
...
function setDepartmentUser(item) {
    ...
    $.getJSON("/Master/GetAllUserInfo", {}, function (data) {
...
function DeleteDepartmentUser(item) {
    ...
    $.getJSON("/Master/GetDepartmentMaster", { SDep: $selectedDept }, function (data) {
